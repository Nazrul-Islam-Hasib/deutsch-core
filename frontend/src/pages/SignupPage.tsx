...
const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('Registration attempt:', { email, password, success });
    if (success) {
      navigate('/');
    }
  };
      ...
      <div className="card w-full max-w-md bg-black/60 backdrop-blur-xl border border-primary/20 shadow-[0_0_50px_rgba(255,124,245,0.1)] z-10">
        <div className="card-body p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-[10px] font-headline tracking-[0.2em] text-secondary/60 uppercase">New Operator Registration</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-headline text-[10px] tracking-widest uppercase text-primary/70">Registration_ID (Email)</span>
              </label>
              <input
                type="email"
            ...
            <div className="form-control w-full">
              <label className="label">
                <span className="label-text font-headline text-[10px] tracking-widest uppercase text-primary/70">Confirm_Security_Key</span>
              </label>
              <input
                type="password"
                placeholder="••••••••"
            ...
            <div className="pt-6">
              <button 
                ...
                disabled={isLoading}
              >
                {isLoading ? 'REGISTERING...' : 'REGISTER_OPERATOR'}
              </button>
            </div>
          </form>
          ...
          <div className="mt-8 text-center">
            <p className="text-[10px] font-headline tracking-widest text-primary/40 uppercase">
              Already authorized?{' '}
              <Link to="/login" className="text-secondary hover:text-secondary/80 transition-colors underline decoration-secondary/30">
                Initialize_Auth
              </Link>
            </p>
          </div>
